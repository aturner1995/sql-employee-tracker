...
const addManager = (db, startPrompt) => {
    db.query(`SELECT * FROM roles WHERE title LIKE '%manager%'`, (err, roleData) => {
        ...
        inquirer.prompt([
            {
                type: 'input',
                message: 'Please provide the manager first name:',
                name: 'firstName'
            },
            {
                type: 'input',
                message: 'Please provide the manager last name:',
                name: 'lastName'
            },
            {
                type: 'list',
                message: 'Please select the manager role:',
                name: 'role',
                choices: roles
            },
        ])
            .then((answer) => {
                ...
                // Insert the new employee data into the database
                db.query(`INSERT INTO employee (first_name, last_name, role_id, manager_id) VALUES 
                    ('${answer.firstName}', '${answer.lastName}', ${selectedRole.id}, ${null})`, (err) => {
                    if (err) {
                        console.log(err);
                        startPrompt();
                        return;
...
module.exports = { addDepartment, addEmployee, addRole, addManager };